...
if __name__ == '__main__':
    ...
    config = utils.load_yaml('predict/config.yaml')
    ...
    for idx, image_path in enumerate(image_paths[int(args.start_index) - 1:], int(args.start_index)):
        ...
        class_scores, class_names = predictor(images=images)
        for i, (class_score, class_name) in enumerate(class_scores, class_names):
